...
- **事件摘要 / 风险提示**：规则引擎基于 FOMC 日历、汇率波动、模型带宽与当日涨跌生成说明，展示在交易点下方；**不改写**上车/下车数字。接口：`GET /api/market-brief`。
- **本场涨跌预测**：按 4 小时场次规则，预测浙商积存金本场收盘相对开盘涨/跌/平（盘中动量 + 短线模型）。接口：`GET /api/guess/status`。
